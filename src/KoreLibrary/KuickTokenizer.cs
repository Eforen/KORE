        ...
        public TokenData readToken()
        {
            ...
            foreach (TokenFinder finder in Spec){
                ...
                if (finder.token == Token.IDENTIFIER)
                {
                    ...
                    // }
                    if (Array.IndexOf(OP_TYPES[Token.OP_R], valueUpper) >= 0) return new TokenData(Token.OP_R, valueUpper);
                    if (Array.IndexOf(OP_TYPES[Token.OP_I], valueUpper) >= 0) return new TokenData(Token.OP_I, valueUpper);
                    if (Array.IndexOf(OP_TYPES[Token.OP_U], valueUpper) >= 0) return new TokenData(Token.OP_U, valueUpper);
                    if (Array.IndexOf(OP_TYPES[Token.OP_B], valueUpper) >= 0) return new TokenData(Token.OP_B, valueUpper);
                    if (Array.IndexOf(OP_TYPES[Token.OP_J], valueUpper) >= 0) return new TokenData(Token.OP_J, valueUpper);
                    if (Array.IndexOf(OP_TYPES[Token.OP_S], valueUpper) >= 0) return new TokenData(Token.OP_S, valueUpper);
                    if (Array.IndexOf(OP_TYPES[Token.OP_CR], valueUpper) >= 0) return new TokenData(Token.OP_CR, valueUpper);
                    if (Array.IndexOf(OP_TYPES[Token.OP_CI], valueUpper) >= 0) return new TokenData(Token.OP_CI, valueUpper);
                    if (Array.IndexOf(OP_TYPES[Token.OP_CSS], valueUpper) >= 0) return new TokenData(Token.OP_CSS, valueUpper);
                    if (Array.IndexOf(OP_TYPES[Token.OP_CIW], valueUpper) >= 0) return new TokenData(Token.OP_CIW, valueUpper);
                    if (Array.IndexOf(OP_TYPES[Token.OP_CL], valueUpper) >= 0) return new TokenData(Token.OP_CL, valueUpper);
                    if (Array.IndexOf(OP_TYPES[Token.OP_CS], valueUpper) >= 0) return new TokenData(Token.OP_CS, valueUpper);
                    if (Array.IndexOf(OP_TYPES[Token.OP_CB], valueUpper) >= 0) return new TokenData(Token.OP_CB, valueUpper);
                    if (Array.IndexOf(OP_TYPES[Token.OP_CJ], valueUpper) >= 0) return new TokenData(Token.OP_CJ, valueUpper);

                    if (Array.IndexOf(OP_TYPES[Token.OP_PSEUDO], valueUpper) >= 0) return new TokenData(Token.OP_PSEUDO, valueUpper);
                }
